...
export async function getDashboardMetrics(): Promise<DashboardMetrics> {
  return withCache(CACHE_KEYS.dashboardMetrics, TTL.dashboard, async () => {
    ...
    const [
      ...
      supabase.from("sites").select("*", { count: "exact", head: true }).in("status", ["at_risk", "non_compliant"]),
...
export async function getRecentAlerts(): Promise<RecentAlert[]> {
  return withCache(CACHE_KEYS.recentAlerts, TTL.alerts, async () => {
    const supabase = await createClient()
...
export async function getSitesSummary(): Promise<SiteSummary[]> {
  return withCache(CACHE_KEYS.sitesSummary, TTL.sites, async () => {
    const supabase = await createClient()
